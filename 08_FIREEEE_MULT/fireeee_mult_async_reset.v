...
*
* Multiplier instance (w/ Asynchronous Reset)
*
* Version: 0.01
* Author : AUDIY
* Date   : 2026/03/29
...
module fireeee_mult_async_reset #(
        ...
        if (IN_REG_EN == 1'b1) begin: genblk_ireg1
            ...
            always @(posedge CLK_I or negedge N_RST_I) begin
                if (N_RST_I == 1'b0) begin
                    /* Asynchronous Reset */
                    CLKS_I_reg <= {(CLKS_WIDTH    ){1'b0}};
                    DATA_I_reg <= {(DATA_BIT_WIDTH){1'b0}};
                    COEF_I_reg <= {(COEF_BIT_WIDTH){1'b0}};
                end else begin
                    CLKS_I_reg <= CLKS_I;
                    DATA_I_reg <= DATA_I;
                    COEF_I_reg <= COEF_I;
    ...
    generate
        if (OUT_REG_EN == 1'b1) begin: genblk_oreg1
            ...
            always @(posedge CLK_I or negedge N_RST_I) begin
                if (N_RST_I == 1'b0) begin
                    /* Asynchronous Reset */
                    CLKS_O_reg <= {(CLKS_WIDTH    ){1'b0}};
                    MULT_O_reg <= {(mult_bit_width){1'b0}};
                end else begin
