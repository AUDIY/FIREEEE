...
*
* Accumulator instance (w/ Synchronous Reset)
*
* Version: 0.03
* Author : AUDIY
* Date   : 2026/04/13
...
*       N_RST_I        : Active Low Synchronous Reset
*
*   Output
...
module fireeee_accum_sync_reset #(
        ...
        if (IN_REG_EN == 1'b1) begin: genblk_ireg1
            ...
            always @(posedge CLK_I) begin
                if (N_RST_I == 1'b0) begin
                    mult_ireg <= {(MULT_WIDTH){1'b0}};
                    clks_ireg <= {(CLKS_WIDTH){1'b0}};
                    edge_ireg <= 1'b0;
                end else begin
                    mult_ireg <= MULT_I;
                    clks_ireg <= CLKS_I;
    ...
    always @(posedge CLK_I) begin
        if (N_RST_I == 1'b0) begin
            clks_reg1 <= {(CLKS_WIDTH){1'b0}};
            clks_reg2 <= {(CLKS_WIDTH){1'b0}};
        end else begin
    ...
    generate
        if (OUT_REG_EN == 1'b1) begin: genblk_oreg1
            ...
            always @(posedge CLK_I) begin
                if (N_RST_I == 1'b0) begin
                    accum_oreg <= {(MULT_WIDTH){1'b0}};
                    clks_oreg  <= {(CLKS_WIDTH){1'b0}};
                end else begin
